...
class WXBot:
    ...
    def init(self):
        url = self.base_uri + '/webwxinit?r=%i&lang=en_US&pass_ticket=%s' % \
        ...
        self.user = dic['User']
        self.sync_key_str = '|'.join([str(keyVal['Key']) +
                                      '_' + str(keyVal['Val']) for keyVal in self.sync_key['List']])
    ...
    def sync_check(self):
        ...
        url = 'https://' + self.sync_host + '.weixin.qq.com/cgi-bin/mmwebwx-bin/synccheck?' + \
              urllib.parse.urlencode(params)
        r = self.session.get(url)
        r.encoding = 'utf-8'
        data = r.text
        pm = re.search('window.synccheck=\{retcode:"(\d+)",selector:"(\d+)"\}', data)
        retcode = pm.group(1)
        selector = pm.group(2)
    ...
    def get_icon(self, icon_id):
        url = self.base_uri + '/webwxgeticon?username=%s&skey=%s' % (icon_id, self.skey)
        r = self.session.get(url)
        data = r.content
        fn = 'img_' + icon_id + '.jpg'
    ...
    def get_head_img(self, img_id):
        url = self.base_uri + '/webwxgetheadimg?username=%s&skey=%s' % (img_id, self.skey)
        r = self.session.get(url)
        data = r.content
        fn = 'img_' + img_id + '.jpg'
        with open(fn, 'wb') as f:
    ...
    def handle_msg(self, r):
        for msg in r['AddMsgList']:
            ...
            if mtype == 51:  # init message
                ...
                content = {'nickname': info['NickName'], 'alias': info['Alias'], 'province': info['Province'],
                           'city': info['City'], 'gender': ['unknown', 'male', 'female'][info['Sex']]}
                if self.DEBUG:
                ...
                content = {'type': appmsgtype[msg['AppMsgType']], 'title': msg['FileName'],
                           'desc': self.search_content('des', content, 'xml'), 'url': msg['Url'],
                ...
                if self.DEBUG:
                    ...
                    print(msg)
            message = {'msg_id': msg_id, 'msg_type_id': msg_type_id, 'content': content, 'user_id': msg['FromUserName'],
                       'user_name': name}
